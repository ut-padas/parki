...
def load_times_from_disk(args, timestamp="latest", version=1):
    ...
    try:
        ...
        raise FileNotFoundError(
            str(e)
            + f"\n please run 'analysis/ewald/time_g2p_methods.py' "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate analytical runtime models "
        "for the G2P kernel and compare with "
        "empirical runtimes on a given device. "
        "Timing results are generated by the "
        "'analysis/ewald/time_g2p_methods.py' file."
    )
    parser.add_argument(
        "--up",
        dest="up",
