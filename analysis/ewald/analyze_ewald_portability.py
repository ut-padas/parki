...
def main(args):
    ...
    for p in range(2):
        ...
        for i, nt in enumerate(nt_list):
            ...
            ax.set_ylabel(r"$\mathrm{\mu s}/N$", fontsize=label_size)
...
def load_times_from_disk(args, timestamp="latest", version=1):
    ...
    try:
        ...
        raise FileNotFoundError(
            str(e)
            + f"\n please run 'analysis/ewald/time_ewald.py' "
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare the full Ewald sum "
        "on different devices. "
        "Timing results are generated by the "
        "'analysis/ewald/time_ewald.py' file. "
        "NOTE: host arch aarch64 corresponds to the grace CPU, "
        "while host arch x86_64 corresponds to the epyc CPU."
    )
    parser.add_argument(
        "--up",
        dest="up",
