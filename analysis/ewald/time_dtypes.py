    ...
    for dtype in ["fp32", "fp64"]:
        ...
        for i, nt in enumerate(nt_list):
            ...
            for j in range(repeats):
                ...
                if i == 0 and j == 0:
                    for key in times:
                        all_times[dtype][key] = np.full(shape=[repeats, len(nt_list)], fill_value=np.inf)
                # Store runtimes
...
def run(args, time_every_step=False, verbosity=0) -> None:
    ...
    if parkipy.utils.get_execution_space(args.device) in [pk.ExecutionSpace.Cuda, pk.ExecutionSpace.HIP]:
        trg = cp.random.rand(3,nt, dtype=dtype)
        src = cp.random.rand(3,nt, dtype=dtype)
    else:
        trg = cp.random.rand(3, nt).astype(dtype) * cp.array(box, dtype=dtype).reshape(3,1)
        src = cp.random.rand(3, ns).astype(dtype) * cp.array(box, dtype=dtype).reshape(3,1)
    ...
    dens = cp.vstack((dens_sl, dens_dl), dtype=dtype).astype(dtype)  # stack densities for ewald call
