...
# automatically from the template file 'parkipy/ewald/_pk_kernels/templates/_cnv_workunits.py'
...
@pk.function
def _k_vector_fp32(M: int, L: pk.float, ups: pk.float, i: int) -> pk.float:
    TWO_PI: pk.float = 6.28318530717958647692529
...
def stokes_kernel_fp32(
    ...
    EIGHT_PI: pk.float = 25.1327412287183459077011
    # See the MATLAB code for details on the method
    # scaling for the global Fourier domain or local pad
    biharm: pk.float = (EIGHT_PI) / (kk * kk)
    C: pk.float = kk / (4.0 * xi * xi)
    screen: pk.float = (1.0 + C) * pk.exp(-C)
    # compute window_m2 on the fly
    b2: pk.float = wsh * wsh
    f1: pk.float = _kaiser_exact_ft_fp32(k0 * k0, b2, whw, ksc)
    f2: pk.float = _kaiser_exact_ft_fp32(k1 * k1, b2, whw, ksc)
    f3: pk.float = _kaiser_exact_ft_fp32(k2 * k2, b2, whw, ksc)
    F: pk.float = f1 * f2 * f3  # tensor product of spatial directions
    if pw == 2:
        F *= F
    window_m2: pk.float = 1.0 / F
    scaling: pk.float = biharm * screen * window_m2
...
@pk.workunit
def laplace_convolution_kernel_fp32(
    ...
    def thread_loop(tid: int):
        ...
        k0: pk.float = 0.0
        if num_locals == 0:
            k0 = _k_vector_fp32(grid[0], box[0], 1.0, i)
        else:
            k0 = _k_vector_fp32(grid[0], box[0], 1.0, locals[i])
        k1: pk.float = _k_vector_fp32(
            grid[1], box[1], ups[0], j + k1_off
        )  # free direction
        k2: pk.float = _k_vector_fp32(grid[2], box[2], ups[1], k)  # free direction
        kk: pk.float = k0 * k0 + k1 * k1 + k2 * k2

        # kernel dependent computations
        FOUR_PI: pk.float = 12.566370614359172954
        C: pk.float = kk / (4.0 * xi * xi)
        screen: pk.float = pk.exp(-C)
        # compute window_m2 on the fly
        b2: pk.float = wsh * wsh
        f1: pk.float = _kaiser_exact_ft_fp32(k0 * k0, b2, whw, ksc)
        f2: pk.float = _kaiser_exact_ft_fp32(k1 * k1, b2, whw, ksc)
        ...
        if pw == 2:
            F *= F
        window_m2: pk.float = 1.0 / F
        scaling: pk.float = FOUR_PI * window_m2 * screen / kk
        H[i][j][k][0] *= scaling
        H[i][j][k][1] *= scaling
...
def stokeslet_convolution_kernel_fp32(
    ...
    def thread_loop(tid: int):
        ...
        scaling: pk.float = stokes_kernel_fp32(kk, k0, k1, k2, xi, wsh, whw, ksc, pw)
...
def stokeslet_convolution_zero_kernel_fp32(
    ...
    PI: pk.float = 3.14159265358979323846264
    FOUR_PI: pk.float = 12.5663706143591729538506
    EIGHT_PI: pk.float = 25.1327412287183459077011
    wid_off: int = team_member.league_rank() * threads
    ...
    def thread_loop(tid: int):
        ...
        k2: pk.float = _k_vector_fp32(grid[2], box[2], ups[1], j)  # free direction
        kk: pk.float = k0 * k0 + k1 * k1 + k2 * k2

        scaling_b: pk.float = 0.0
        if periodicity == 1:
            # See the MATLAB code for details on the method
            ...
            if pw == 2:
                F *= F
            window_m2: pk.float = 1.0 / F
            combo: pk.float = screen * window_m2
            scaling_b = biharm * combo
            ...
            if vico_var == 2:
                ...
                if (i + k1_off) == 0 and j == 0:
                    ...
                    harm = FOUR_PI * (1.0 - BJ0) / kk
                scaling_H: pk.float = 2 * harm * combo
                H1[i][j][0] = scaling_H * H1[i][j][0]
                H1[i][j][1] = scaling_H * H1[i][j][1]
        elif periodicity == 3:
            scaling_b = stokes_kernel_fp32(kk, k0, k1, k2, xi, wsh, whw, ksc, pw)

        # relate harmonic/biharmonic to stokeslet
        SKK: pk.float = -kk * scaling_b
        ...
        # always use biharmonic for the other two components
        k_dot_H_0: pk.float = k1 * H2[i][j][0] + k2 * H3[i][j][0]
        k_dot_H_1: pk.float = k1 * H2[i][j][1] + k2 * H3[i][j][1]
        k_dot_H_0 = scaling_b * k_dot_H_0
        k_dot_H_1 = scaling_b * k_dot_H_1
        H2[i][j][0] = SKK * H2[i][j][0] + k_dot_H_0 * k1
        H2[i][j][1] = SKK * H2[i][j][1] + k_dot_H_1 * k1
...
@pk.workunit
def stresslet_convolution_kernel_fp32(
    team_member: pk.TeamMember,
    H11,
    H21,
    H31,
    ...
    def thread_loop(tid: int):
        ...
        k_dot_H_dot_k_0: pk.float = k0 * H_dot_k0_0 + k1 * H_dot_k1_0 + k2 * H_dot_k2_0
        k_dot_H_dot_k_1: pk.float = k0 * H_dot_k0_1 + k1 * H_dot_k1_1 + k2 * H_dot_k2_1
        k_dot_H1_0: pk.float = (
            k0 * H11[i][j][k][0] + k1 * H21[i][j][k][0] + k2 * H31[i][j][k][0]
        )
        k_dot_H1_1: pk.float = (
        ...
        trace_H_0: pk.float = H11[i][j][k][0] + H22[i][j][k][0] + H33[i][j][k][0]
        trace_H_1: pk.float = H11[i][j][k][1] + H22[i][j][k][1] + H33[i][j][k][1]
        vp1_0: pk.float = (
            H_dot_k0_1 + k_dot_H1_1 + k0 * trace_H_1
        ) * kk - 2.0 * k0 * k_dot_H_dot_k_1
        vp1_1: pk.float = (
...
def stresslet_convolution_zero_kernel_fp32(
    ...
    def thread_loop(tid: int):
        ...
        # relate harmonic/biharmonic to stresslet
        H_dot_k0_0: pk.float = H12[i][j][0] * k1 + H13[i][j][0] * k2
        H_dot_k0_1: pk.float = H12[i][j][1] * k1 + H13[i][j][1] * k2
        H_dot_k1_0: pk.float = H22[i][j][0] * k1 + H23[i][j][0] * k2
        H_dot_k1_1: pk.float = H22[i][j][1] * k1 + H23[i][j][1] * k2
        H_dot_k2_0: pk.float = H32[i][j][0] * k1 + H33[i][j][0] * k2
        H_dot_k2_1: pk.float = H32[i][j][1] * k1 + H33[i][j][1] * k2
        k_dot_H_dot_k_0: pk.float = k1 * H_dot_k1_0 + k2 * H_dot_k2_0
        k_dot_H_dot_k_1: pk.float = k1 * H_dot_k1_1 + k2 * H_dot_k2_1
        scaling_H: pk.float = 0.0
        trace_H_0: pk.float = 0.0
        trace_H_1: pk.float = 0.0
        if vico_var == 2:
            harm: pk.float = 0.0
            if (i + k1_off) == 0 and j == 0:
                harm = PI * (gR * gR)
            ...
            trace_H_1 = H11[i][j][1] + H22[i][j][1] + H33[i][j][1]
        # relation between stresslet and biharmonic
        k_dot_H1_0: pk.float = k1 * H21[i][j][0] + k2 * H31[i][j][0]
        k_dot_H1_1: pk.float = k1 * H21[i][j][1] + k2 * H31[i][j][1]
        vp1_0: pk.float = 0.0
        vp1_1: pk.float = 0.0
        if vico_var != 2:
            vp1_0 = (
            ...
            H11[i][j][0] = scaling_b * vp1_1
            H11[i][j][1] = -scaling_b * vp1_0
        k_dot_H2_0: pk.float = k1 * H22[i][j][0] + k2 * H32[i][j][0]
        k_dot_H2_1: pk.float = k1 * H22[i][j][1] + k2 * H32[i][j][1]
        vp2_0: pk.float = (
            H_dot_k1_0 + k_dot_H2_0 + k1 * trace_H_0
        ) * kk - 2 * k1 * k_dot_H_dot_k_0
        vp2_1: pk.float = (
        ...
        H21[i][j][0] = scaling_b * vp2_1
        H21[i][j][1] = -scaling_b * vp2_0
        k_dot_H3_0: pk.float = k1 * H23[i][j][0] + k2 * H33[i][j][0]
        k_dot_H3_1: pk.float = k1 * H23[i][j][1] + k2 * H33[i][j][1]
        vp3_0: pk.float = (
            H_dot_k2_0 + k_dot_H3_0 + k2 * trace_H_0
        ) * kk - 2 * k2 * k_dot_H_dot_k_0
        vp3_1: pk.float = (
        ...
        # relation between stresslet and harmonic
        if vico_var == 2:
            H11i_0: pk.float = H11[i][j][0]
            H11i_1: pk.float = H11[i][j][1]
            vp1_0 = k0 * H11i_0 + k_dot_H1_0 + H_dot_k0_0
            vp1_1 = k0 * H11i_1 + k_dot_H1_1 + H_dot_k0_1
            H11[i][j][0] = -2 * scaling_H * vp1_1
            H11[i][j][1] = 2 * scaling_H * vp1_0
...
@pk.workunit
def convolution_sum_sl_dl_fp32(
    team_member: pk.TeamMember,
    H1,
    H2,
    H3,
...
@pk.function
def _k_vector_fp64(M: int, L: pk.double, ups: pk.double, i: int) -> pk.double:
...
@pk.workunit
def laplace_convolution_kernel_fp64(
    team_member: pk.TeamMember,
    H,
    grid_size_1: int,
    grid_size_2: int,
...
@pk.workunit
def stokeslet_convolution_kernel_fp64(
    team_member: pk.TeamMember,
    H1,
    H2,
    H3,
...
@pk.workunit
def stokeslet_convolution_zero_kernel_fp64(
    team_member: pk.TeamMember,
    H1,
    H2,
    H3,
...
@pk.workunit
def stresslet_convolution_kernel_fp64(
    team_member: pk.TeamMember,
    H11,
    H21,
    H31,
...
@pk.workunit
def stresslet_convolution_zero_kernel_fp64(
    team_member: pk.TeamMember,
    H11,
    H21,
    H31,
...
@pk.workunit
def convolution_sum_sl_dl_fp64(
    team_member: pk.TeamMember,
    H1,
    H2,
    H3,
